...
class AgentProfileTests(TestCase):
    ...
    def setUp(self):
        ...
        path = '%s?%s' % (reverse('lrs:agent_profile'), urllib.urlencode(self.testparams1))
        self.testprofile1 = {"test":"put profile 1","obj":{"agent":"test"}}
        self.put1 = self.client.put(path, self.testprofile1, content_type=self.content_type, Authorization=self.auth, X_Experience_API_Version=settings.XAPI_VERSION)

        self.testparams2 = {"profileId": self.testprofileId2, "agent": self.testagent}
        path = '%s?%s' % (reverse('lrs:agent_profile'), urllib.urlencode(self.testparams2))
        self.testprofile2 = {"test":"put profile 2","obj":{"agent":"test"}}
        self.put2 = self.client.put(path, self.testprofile2, content_type=self.content_type, Authorization=self.auth, X_Experience_API_Version=settings.XAPI_VERSION)

        self.testparams3 = {"profileId": self.testprofileId3, "agent": self.testagent}
        path = '%s?%s' % (reverse('lrs:agent_profile'), urllib.urlencode(self.testparams3))
        self.testprofile3 = {"test":"put profile 3","obj":{"agent":"test"}}
        self.put3 = self.client.put(path, self.testprofile3, content_type=self.content_type, Authorization=self.auth, X_Experience_API_Version=settings.XAPI_VERSION)

        self.testparams4 = {"profileId": self.otherprofileId1, "agent": self.otheragent}
        path = '%s?%s' % (reverse('lrs:agent_profile'), urllib.urlencode(self.testparams4))
        self.otherprofile1 = {"test":"put profile 1","obj":{"agent":"other"}}
        self.put4 = self.client.put(path, self.otherprofile1, content_type=self.content_type, Authorization=self.auth, X_Experience_API_Version=settings.XAPI_VERSION)

    def tearDown(self):
        self.client.delete(reverse('lrs:agent_profile'), self.testparams1, Authorization=self.auth, X_Experience_API_Version=settings.XAPI_VERSION)
        self.client.delete(reverse('lrs:agent_profile'), self.testparams2, Authorization=self.auth, X_Experience_API_Version=settings.XAPI_VERSION)
        self.client.delete(reverse('lrs:agent_profile'), self.testparams3, Authorization=self.auth, X_Experience_API_Version=settings.XAPI_VERSION)
        self.client.delete(reverse('lrs:agent_profile'), self.testparams4, Authorization=self.auth, X_Experience_API_Version=settings.XAPI_VERSION)

    def test_get_agent_not_found(self):
        a = '{"mbox":"mailto:notfound@example.com"}'
        p = 'http://agent.not.found'
        param = {"profileId": p, "agent": a}
        r = self.client.get(reverse('lrs:agent_profile'), param, Authorization=self.auth, X_Experience_API_Version=settings.XAPI_VERSION)
    ...
    def test_put_etag_missing_on_change(self):
        ...
        r = self.client.get(reverse('lrs:agent_profile'), self.testparams1, Authorization=self.auth, X_Experience_API_Version=settings.XAPI_VERSION)
    ...
    def test_put_etag_if_none_match_good(self):
        ...
        r = self.client.get(reverse('lrs:agent_profile'), params, Authorization=self.auth, X_Experience_API_Version=settings.XAPI_VERSION)
        self.assertEqual(r.status_code, 200)
        robj = ast.literal_eval(r.content)
        self.assertEqual(robj['test'], profile['test'])
        self.assertEqual(robj['obj']['agent'], profile['obj']['agent'])

        r = self.client.delete(reverse('lrs:agent_profile'), params, Authorization=self.auth, X_Experience_API_Version=settings.XAPI_VERSION)
    ...
    def test_get_invalid_agent_structure(self):
        r = self.client.get(reverse('lrs:agent_profile'), {"profileId": self.testprofileId1, "agent": "wrong"}, Authorization=self.auth, X_Experience_API_Version=settings.XAPI_VERSION)
        self.assertEqual(r.status_code, 400)
        self.assertEqual(r.content, "agent param for agent profile is not valid")
        
    def test_get_invalid_agent(self):
        r = self.client.get(reverse('lrs:agent_profile'), {"profileId": self.testprofileId1, "agent": {"mbox":"foo"}}, Authorization=self.auth, X_Experience_API_Version=settings.XAPI_VERSION)
        self.assertEqual(r.status_code, 400)
        self.assertEqual(r.content, "agent param for agent profile is not valid")
    ...
    def test_get(self):
        ...
        r2 = self.client.get(reverse('lrs:agent_profile'), self.testparams2, Authorization=self.auth, X_Experience_API_Version=settings.XAPI_VERSION)
        ...
        r3 = self.client.get(reverse('lrs:agent_profile'), self.testparams3, Authorization=self.auth, X_Experience_API_Version=settings.XAPI_VERSION)
        ...
        r4 = self.client.get(reverse('lrs:agent_profile'), self.testparams4, Authorization=self.auth, X_Experience_API_Version=settings.XAPI_VERSION)
    ...
    def test_get_no_params(self):
        r = self.client.get(reverse('lrs:agent_profile'), Authorization=self.auth, X_Experience_API_Version=settings.XAPI_VERSION)
        self.assertEqual(r.status_code, 400)
    ...
    def test_get_agent_since(self):
        ...
        response = self.client.put(path, profile, content_type=self.content_type, updated=updated, Authorization=self.auth, X_Experience_API_Version=settings.XAPI_VERSION)
        self.assertEqual(response.status_code, 204)
        ...
        r2 = self.client.get(reverse('lrs:agent_profile'), params2, Authorization=self.auth, X_Experience_API_Version=settings.XAPI_VERSION)
        self.assertNotIn(prof_id, r2.content)

        self.client.delete(reverse('lrs:agent_profile'), params, Authorization=self.auth, X_Experience_API_Version=settings.XAPI_VERSION)
    ...
    def test_get_agent_since_tz(self):
        ...
        path2 = '%s?%s' % (reverse('lrs:agent_profile'), urllib.urlencode(params2))

        profile2 = {"test3":"agent profile since time: %s" % updated2,"obj":{"agent":"test"}}
        response = self.client.put(path2, profile2, content_type=self.content_type, updated=updated2, Authorization=self.auth, X_Experience_API_Version=settings.XAPI_VERSION)
        self.assertEqual(response.status_code, 204)
        ...
        r = self.client.get(reverse('lrs:agent_profile'), par, Authorization=self.auth, X_Experience_API_Version=settings.XAPI_VERSION)

        self.assertNotIn(prof_id, r.content)
        self.assertIn(prof_id2, r.content)

        self.client.delete(reverse('lrs:agent_profile'), params, Authorization=self.auth, X_Experience_API_Version=settings.XAPI_VERSION)
        self.client.delete(reverse('lrs:agent_profile'), params2, Authorization=self.auth, X_Experience_API_Version=settings.XAPI_VERSION)

    def test_post_put_delete(self):
        prof_id = "http://deleteme.too"
        path = '%s?%s' % (reverse('lrs:agent_profile'), urllib.urlencode({"method":"PUT"}))
        ...
        self.assertEqual(response.status_code, 204)
        r = self.client.get(reverse('lrs:agent_profile'), {"profileId": prof_id, "agent": self.testagent}, Authorization=self.auth, X_Experience_API_Version=settings.XAPI_VERSION)
        self.assertEqual(r.status_code, 200)
        import ast
        ...
        path = '%s?%s' % (reverse('lrs:agent_profile'), urllib.urlencode({"method":"DELETE"}))
        r = self.client.post(path, thedata, content_type="application/x-www-form-urlencoded", Authorization=self.auth, X_Experience_API_Version=settings.XAPI_VERSION)
        self.assertEqual(r.status_code, 204)
    ...
    def test_group_as_agent(self):
        ...
        path = '%s?%s' % (reverse('lrs:agent_profile'), urllib.urlencode(testparams1))
        testprofile = {"test":"put profile - group as agent","obj":{"agent":"group"}}
        put1 = self.client.put(path, testprofile, content_type=self.content_type, Authorization=self.auth, X_Experience_API_Version=settings.XAPI_VERSION)

        self.assertEqual(put1.status_code, 204)

        getr = self.client.get(reverse('lrs:agent_profile'), testparams1, Authorization=self.auth, X_Experience_API_Version=settings.XAPI_VERSION)
        ...
        self.client.delete(reverse('lrs:agent_profile'), testparams1, Authorization=self.auth, X_Experience_API_Version=settings.XAPI_VERSION)
