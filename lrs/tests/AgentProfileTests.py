...
class AgentProfileTests(TestCase):
    ...
    def test_post_put_delete(self):
        ...
        thedata = "profileId=%s&agent=%s&content=%s&Authorization=%s&Content-Type=application/json&X-Experience-API-Version=0.95" % (prof_id, self.testagent, content, self.auth)
        ...
        thedata = "profileId=%s&agent=%s&Authorization=%s&X-Experience-API-Version=0.95" % (prof_id, self.testagent, self.auth)
