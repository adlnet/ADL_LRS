...
from ..views import agent_profile
...
class AgentProfileTests(TestCase):
    ...
    def setUp(self):
        ...
        path = '%s?%s' % (reverse(agent_profile), urllib.urlencode(self.testparams1))
        ...
        path = '%s?%s' % (reverse(agent_profile), urllib.urlencode(self.testparams2))
        ...
        path = '%s?%s' % (reverse(agent_profile), urllib.urlencode(self.testparams3))
        ...
        path = '%s?%s' % (reverse(agent_profile), urllib.urlencode(self.testparams4))
    ...
    def tearDown(self):
        self.client.delete(reverse(agent_profile), self.testparams1, Authorization=self.auth, X_Experience_API_Version=settings.XAPI_VERSION)
        self.client.delete(reverse(agent_profile), self.testparams2, Authorization=self.auth, X_Experience_API_Version=settings.XAPI_VERSION)
        self.client.delete(reverse(agent_profile), self.testparams3, Authorization=self.auth, X_Experience_API_Version=settings.XAPI_VERSION)
        self.client.delete(reverse(agent_profile), self.testparams4, Authorization=self.auth, X_Experience_API_Version=settings.XAPI_VERSION)

    def test_get_agent_not_found(self):
        a = '{"mbox":"mailto:notfound@example.com"}'
        p = 'http://agent.not.found'
        param = {"profileId": p, "agent": a}
        r = self.client.get(reverse(agent_profile), param, Authorization=self.auth, X_Experience_API_Version=settings.XAPI_VERSION)
    ...
    def test_put_etag_if_none_match_good(self):
        ...
        r = self.client.get(reverse(agent_profile), params, Authorization=self.auth, X_Experience_API_Version=settings.XAPI_VERSION)
        self.assertEqual(r.status_code, 200)
        robj = ast.literal_eval(r.content)
        self.assertEqual(robj['test'], profile['test'])
        self.assertEqual(robj['obj']['agent'], profile['obj']['agent'])

        r = self.client.delete(reverse(agent_profile), params, Authorization=self.auth, X_Experience_API_Version=settings.XAPI_VERSION)
    ...
    def test_get_invalid_agent_structure(self):
        r = self.client.get(reverse(agent_profile), {"profileId": self.testprofileId1, "agent": "wrong"}, Authorization=self.auth, X_Experience_API_Version=settings.XAPI_VERSION)
        self.assertEqual(r.status_code, 400)
        self.assertEqual(r.content, "agent param for agent profile is not valid")
        
    def test_get_invalid_agent(self):
        r = self.client.get(reverse(agent_profile), {"profileId": self.testprofileId1, "agent": {"mbox":"foo"}}, Authorization=self.auth, X_Experience_API_Version=settings.XAPI_VERSION)
        self.assertEqual(r.status_code, 400)
        self.assertEqual(r.content, "agent param for agent profile is not valid")
    ...
    def test_get_no_params(self):
        r = self.client.get(reverse(agent_profile), Authorization=self.auth, X_Experience_API_Version=settings.XAPI_VERSION)
        self.assertEqual(r.status_code, 400)
    ...
    def test_get_agent_since(self):
        ...
        r2 = self.client.get(reverse(agent_profile), params2, Authorization=self.auth, X_Experience_API_Version=settings.XAPI_VERSION)
        self.assertNotIn(prof_id, r2.content)

        self.client.delete(reverse(agent_profile), params, Authorization=self.auth, X_Experience_API_Version=settings.XAPI_VERSION)
    ...
    def test_get_agent_since_tz(self):
        ...
        r = self.client.get(reverse(agent_profile), par, Authorization=self.auth, X_Experience_API_Version=settings.XAPI_VERSION)

        self.assertNotIn(prof_id, r.content)
        self.assertIn(prof_id2, r.content)

        self.client.delete(reverse(agent_profile), params, Authorization=self.auth, X_Experience_API_Version=settings.XAPI_VERSION)
        self.client.delete(reverse(agent_profile), params2, Authorization=self.auth, X_Experience_API_Version=settings.XAPI_VERSION)

    def test_post_put_delete(self):
        prof_id = "http://deleteme.too"
        path = '%s?%s' % (reverse(agent_profile), urllib.urlencode({"method":"PUT"}))
        ...
        self.assertEqual(response.status_code, 204)
        r = self.client.get(reverse(agent_profile), {"profileId": prof_id, "agent": self.testagent}, Authorization=self.auth, X_Experience_API_Version=settings.XAPI_VERSION)
        self.assertEqual(r.status_code, 200)
        import ast
        ...
        path = '%s?%s' % (reverse(agent_profile), urllib.urlencode({"method":"DELETE"}))
    ...
    def test_group_as_agent(self):
        ...
        path = '%s?%s' % (reverse(agent_profile), urllib.urlencode(testparams1))
        ...
        getr = self.client.get(reverse(agent_profile), testparams1, Authorization=self.auth, X_Experience_API_Version=settings.XAPI_VERSION)
        ...
        self.client.delete(reverse(agent_profile), testparams1, Authorization=self.auth, X_Experience_API_Version=settings.XAPI_VERSION)
