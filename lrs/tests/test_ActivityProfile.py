...
class ActivityProfileTests(TestCase):
    ...
    def setUp(self):
        ...
        path = '%s?%s' % (reverse('lrs:activity_profile'), urllib.urlencode(self.testparams1))
        ...
        path = '%s?%s' % (reverse('lrs:activity_profile'), urllib.urlencode(self.testparams2))
        ...
        path = '%s?%s' % (reverse('lrs:activity_profile'), urllib.urlencode(self.testparams3))
        ...
        path = '%s?%s' % (reverse('lrs:activity_profile'), urllib.urlencode(self.testparams4))
        ...
        path = '%s?%s' % (reverse('lrs:activity_profile'), urllib.urlencode(self.testparams5))
    ...
    def tearDown(self):
        self.client.delete(reverse('lrs:activity_profile'), self.testparams1, Authorization=self.auth, X_Experience_API_Version=settings.XAPI_VERSION)
        self.client.delete(reverse('lrs:activity_profile'), self.testparams2, Authorization=self.auth, X_Experience_API_Version=settings.XAPI_VERSION)
        self.client.delete(reverse('lrs:activity_profile'), self.testparams3, Authorization=self.auth, X_Experience_API_Version=settings.XAPI_VERSION)
        self.client.delete(reverse('lrs:activity_profile'), self.testparams4, Authorization=self.auth, X_Experience_API_Version=settings.XAPI_VERSION)    
        self.client.delete(reverse('lrs:activity_profile'), self.testparams5, Authorization=self.auth, X_Experience_API_Version=settings.XAPI_VERSION)    
    ...
    def test_put_no_activityId(self):
        put = self.client.put(reverse('lrs:activity_profile'), {'profileId':'10'},content_type=self.content_type, Authorization=self.auth, X_Experience_API_Version=settings.XAPI_VERSION)
        self.assertEquals(put.content, 'Error -- activity_profile - method = PUT, but activityId parameter missing..')

    def test_put_no_profileId(self):
        testparams = {'activityId':'act:act:act'}
        path = '%s?%s' % (reverse('lrs:activity_profile'), urllib.urlencode(testparams))
    ...
    def test_put_etag_missing_on_change(self):
        ...
        r = self.client.get(reverse('lrs:activity_profile'), self.testparams1, X_Experience_API_Version=settings.XAPI_VERSION, Authorization=self.auth)
    ...
    def test_put_etag_if_none_match_good(self):
        ...
        path = '%s?%s' % (reverse('lrs:activity_profile'), urllib.urlencode(params))
        ...
        self.assertEqual(response.status_code, 204)
        r = self.client.get(reverse('lrs:activity_profile'), params, X_Experience_API_Version=settings.XAPI_VERSION, Authorization=self.auth)
        self.assertEqual(r.status_code, 200)
        robj = ast.literal_eval(r.content)
        self.assertEqual(robj['test'], profile['test'])
        self.assertEqual(robj['obj']['activity'], profile['obj']['activity'])

        r = self.client.delete(reverse('lrs:activity_profile'), params, Authorization=self.auth, X_Experience_API_Version=settings.XAPI_VERSION)
    ...
    def test_get_activity_only(self):
        response = self.client.get(reverse('lrs:activity_profile'), {'activityId':self.test_activityId2}, X_Experience_API_Version=settings.XAPI_VERSION, Authorization=self.auth)
        ...
        self.client.delete(reverse('lrs:activity_profile'), params, Authorization=self.auth, X_Experience_API_Version=settings.XAPI_VERSION)
    ...
    def test_get_activity_profileId_no_auth(self):
        response = self.client.get(reverse('lrs:activity_profile'), {'activityId':self.test_activityId1,'profileId':self.testprofileId1}, X_Experience_API_Version=settings.XAPI_VERSION)
        self.assertEqual(response.status_code, 400)

    def test_get_activity_profileId_activity_dne(self):
        response = self.client.get(reverse('lrs:activity_profile'), {'activityId':'http://actID','profileId':self.testprofileId1}, X_Experience_API_Version=settings.XAPI_VERSION, Authorization=self.auth)
    ...
    def test_get_activity_since_tz(self):
        ...
        response = self.client.get(reverse('lrs:activity_profile'), {'activityId': actid,'since':since}, X_Experience_API_Version=settings.XAPI_VERSION, Authorization=self.auth)
        self.assertEqual(response.status_code, 200)
    ...
    def test_get_no_activityId_with_profileId(self):
        response = self.client.get(reverse('lrs:activity_profile'), {'profileId': self.testprofileId3}, X_Experience_API_Version=settings.XAPI_VERSION, Authorization=self.auth)
        self.assertEqual(response.status_code, 400)
    ...
    def test_get_no_activityId_with_since(self):
        ...
        response = self.client.get(reverse('lrs:activity_profile'), {'since':since}, X_Experience_API_Version=settings.XAPI_VERSION, Authorization=self.auth)
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.content, 'Error -- activity_profile - method = GET, but activityId parameter missing..')
    
    def test_delete(self):
        response = self.client.delete(reverse('lrs:activity_profile'), {'activityId':self.other_activityId, 'profileId':self.otherprofileId1}, Authorization=self.auth, X_Experience_API_Version=settings.XAPI_VERSION)
        self.assertEqual(response.status_code, 204)
    ...
    def test_cors_put(self):
        ...
        path = path = '%s?%s' % (reverse('lrs:activity_profile'), urllib.urlencode({"method":"PUT"}))
        ...
        get1 = self.client.get(reverse('lrs:activity_profile'), testparams1, Authorization=self.auth, X_Experience_API_Version=settings.XAPI_VERSION)
        self.assertEqual(get1.status_code, 200)
        
        import ast
        c = ast.literal_eval(get1.content)
        self.assertEqual(c['test'], content['test'])     
        self.client.delete(reverse('lrs:activity_profile'), testparams1, Authorization=self.auth, X_Experience_API_Version=settings.XAPI_VERSION)
    ...
    def test_cors_put_etag(self):
        ...
        self.assertEqual(put1.status_code, 204)
        path = '%s?%s' % (reverse('lrs:activity_profile'), urllib.urlencode({"method":"PUT"}))
        ...
        self.assertEqual(response.status_code, 204)
        r = self.client.get(reverse('lrs:activity_profile'), {'activityId': aid, 'profileId': pid}, X_Experience_API_Version=settings.XAPI_VERSION, Authorization=self.auth)
        self.assertEqual(r.status_code, 200)
        import ast
        c = ast.literal_eval(r.content)
        self.assertEqual(c['test'], content['test'])

        self.client.delete(reverse('lrs:activity_profile'), {'activityId': aid, 'profileId': pid}, Authorization=self.auth, X_Experience_API_Version=settings.XAPI_VERSION)
    ...
    def test_tetris_snafu(self):
        ...
        r = self.client.get(reverse('lrs:activity_profile'), {'activityId': "act:tetris.snafu", 'profileId': "http://test.tetris/"}, X_Experience_API_Version=settings.XAPI_VERSION, Authorization=self.auth)
        self.assertEqual(r.status_code, 200)
