...
class AgentsTests(TestCase):
    ...
    def test_get_no_agents(self):
        ...
        response = self.client.get(reverse(agents), {'agent':agent}, Authorization=self.auth, X_Experience_API_Version="1.0.0")
    ...
    def test_get(self):
        ...
        response = self.client.get(reverse(agents), {'agent':a}, Authorization=self.auth, X_Experience_API_Version="1.0.0")
    ...
    def test_head(self):
        a = json.dumps({"name":"me","mbox":"mailto:me@example.com"})
        Agent.objects.retrieve_or_create(**json.loads(a))
        response = self.client.head(reverse(agents), {'agent':a}, Authorization=self.auth, X_Experience_API_Version="1.0.0")
        self.assertEqual(response.content, '')
        self.assertIn('content-length', response._headers)

    def test_get_no_agent(self):
        response = self.client.get(reverse(agents), Authorization=self.auth, X_Experience_API_Version="1.0.0")
        self.assertEqual(response.status_code, 400)
    
    def test_post(self):
        agent = json.dumps({"name":"me","mbox":"mailto:me@example.com"})
        response = self.client.post(reverse(agents), {'agent':agent},content_type='application/x-www-form-urlencoded', Authorization=self.auth, X_Experience_API_Version="1.0.0")
