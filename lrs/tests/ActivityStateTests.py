...
class ActivityStateTests(TestCase):
    ...
    def setUp(self):
        ...
        self.put1 = self.client.put(path, self.teststate1, content_type=self.content_type, Authorization=self.auth, X_Experience_API_Version="1.0")
        ...
        self.put2 = self.client.put(path, self.teststate2, content_type=self.content_type, Authorization=self.auth, X_Experience_API_Version="1.0")
        ...
        self.put3 = self.client.put(path, self.teststate3, content_type=self.content_type, Authorization=self.auth, X_Experience_API_Version="1.0")
        ...
        self.put4 = self.client.put(path, self.teststate4, content_type=self.content_type, Authorization=self.auth, X_Experience_API_Version="1.0")

    def tearDown(self):
        self.client.delete(self.url, self.testparams1, Authorization=self.auth, X_Experience_API_Version="1.0")
        self.client.delete(self.url, self.testparams2, Authorization=self.auth, X_Experience_API_Version="1.0")
        self.client.delete(self.url, self.testparams3, Authorization=self.auth, X_Experience_API_Version="1.0")
        self.client.delete(self.url, self.testparams4, Authorization=self.auth, X_Experience_API_Version="1.0")
    ...
    def test_put_no_existing_activity(self):
        ...
        put = self.client.put(path, teststate, content_type=self.content_type, Authorization=self.auth, X_Experience_API_Version="1.0")
    ...
    def test_put_with_registrationId(self):
        ...
        put1 = self.client.put(path, teststateregid, content_type=self.content_type, Authorization=self.auth, X_Experience_API_Version="1.0")

        self.assertEqual(put1.status_code, 204)
        self.assertEqual(put1.content, '')
        # also testing get w/ registration id
        r = self.client.get(self.url, testparamsregid, X_Experience_API_Version="1.0", Authorization=self.auth)
        ...
        del_r = self.client.delete(self.url, testparamsregid, Authorization=self.auth, X_Experience_API_Version="1.0")
    ...
    def test_put_without_auth(self):
        ...
        path = '%s?%s' % (self.url, urllib.urlencode(testparamsregid))
        teststateregid = {"test":"put activity state w/ registrationId","obj":{"agent":"test"}}
        put1 = self.client.put(path, teststateregid, content_type=self.content_type, X_Experience_API_Version="1.0")
    ...
    def test_put_etag_conflict_if_none_match(self):
        ...
        path = '%s?%s' % (self.url, urllib.urlencode(self.testparams1))
        r = self.client.put(path, teststateetaginm, content_type=self.content_type, If_None_Match='*', Authorization=self.auth, X_Experience_API_Version="1.0")
        self.assertEqual(r.status_code, 412)
        self.assertEqual(r.content, 'Resource detected')

        r = self.client.get(self.url, self.testparams1, X_Experience_API_Version="1.0", Authorization=self.auth)
        self.assertEqual(r.status_code, 200)
    ...
    def test_put_etag_conflict_if_match(self):
        ...
        path = '%s?%s' % (self.url, urllib.urlencode(self.testparams1))
        r = self.client.put(path, teststateetagim, content_type=self.content_type, If_Match=new_etag, Authorization=self.auth, X_Experience_API_Version="1.0")
        self.assertEqual(r.status_code, 412)
    ...
    def test_put_etag_missing_on_change(self):
        teststateetagim = {'test': 'etag no conflict - if match good hash', 'obj': {'agent': 'test'}}
        path = '%s?%s' % (self.url, urllib.urlencode(self.testparams1))
        r = self.client.put(path, teststateetagim, content_type=self.content_type, Authorization=self.auth, X_Experience_API_Version="1.0")
    ...
    def test_put_without_activityid(self):
        ...
        put1 = self.client.put(path, teststatebad, content_type=self.content_type, Authorization=self.auth, X_Experience_API_Version="1.0")
    ...
    def test_get(self):
        ...
        r2 = self.client.get(self.url, self.testparams2, X_Experience_API_Version="1.0", Authorization=self.auth)
        ...
        r3 = self.client.get(self.url, self.testparams3, X_Experience_API_Version="1.0", Authorization=self.auth)
        ...
        r4 = self.client.get(self.url, self.testparams4, X_Experience_API_Version="1.0", Authorization=auth)
        ...
        # r5 = self.client.get(self.url, self.testparams3, X_Experience_API_Version="1.0", Authorization=auth)
    ...
    def test_get_no_existing_id(self):
        ...
        r = self.client.get(self.url, testparams, X_Experience_API_Version="1.0", Authorization=self.auth)
        self.assertEqual(r.status_code, 404)


    def test_get_ids(self):
        params = {"activityId": self.activityId, "agent": self.testagent}
        r = self.client.get(self.url, params, X_Experience_API_Version="1.0", Authorization=self.auth)
        self.assertEqual(r.status_code, 200)
    ...
    def test_get_with_since(self):
        ...
        put1 = self.client.put(path, teststatesince, content_type=self.content_type, updated=updated.isoformat(), Authorization=self.auth, X_Experience_API_Version="1.0")

        self.assertEqual(put1.status_code, 204)
        self.assertEqual(put1.content, '')
        
        r = self.client.get(self.url, testparamssince, X_Experience_API_Version="1.0", Authorization=self.auth)
        self.assertEqual(r.status_code, 200)
        ...
        r = self.client.get(self.url, params2, X_Experience_API_Version="1.0", Authorization=self.auth)
        self.assertEqual(r.status_code, 200)
        self.assertIn(self.stateId, r.content)
        self.assertIn(self.stateId2, r.content)
        ...
        del_r = self.client.delete(self.url, testparamssince, Authorization=self.auth, X_Experience_API_Version="1.0")
    ...
    def test_get_with_since_tz(self):
        ...
        put2 = self.client.put(path, teststatesince2, content_type=self.content_type, updated=updated_tz, Authorization=self.auth, X_Experience_API_Version="1.0")

        self.assertEqual(put2.status_code, 204)
        self.assertEqual(put2.content, '')
        
        r2 = self.client.get(self.url, testparamssince2, X_Experience_API_Version="1.0", Authorization=self.auth)
        self.assertEqual(r2.status_code, 200)
        ...
        del_r = self.client.delete(self.url, testparamssince, Authorization=self.auth, X_Experience_API_Version="1.0")
        del_r = self.client.delete(self.url, testparamssince2, Authorization=self.auth, X_Experience_API_Version="1.0")
    ...
    def test_get_with_since_and_regid(self):
        ...
        put2 = self.client.put(path, teststatesince2, content_type=self.content_type, updated=updated.isoformat(), Authorization=self.auth, X_Experience_API_Version="1.0")
        ...
        put3 = self.client.put(path, teststatesince3, content_type=self.content_type, Authorization=self.auth, X_Experience_API_Version="1.0")

        self.assertEqual(put3.status_code, 204)
        self.assertEqual(put3.content, '')

        r3 = self.client.get(self.url, testparamssince3, X_Experience_API_Version="1.0", Authorization=self.auth)
        self.assertEqual(r3.status_code, 200)
        ...
        self.client.delete(self.url, testparamssince, Authorization=self.auth, X_Experience_API_Version="1.0")
        self.client.delete(self.url, testparamssince2, Authorization=self.auth, X_Experience_API_Version="1.0")
        self.client.delete(self.url, testparamssince3, Authorization=self.auth, X_Experience_API_Version="1.0")
    ...
    def test_delete_without_activityid(self):
        ...
        f_r = self.client.delete(self.url, {"registrationId": self.registrationId, "stateId": self.stateId, "agent": self.testagent}, Authorization=self.auth, X_Experience_API_Version="1.0")
    ...
    def test_delete_without_agent(self):
        ...
        f_r = self.client.delete(self.url, {"registrationId": self.registrationId, "stateId": self.stateId, "activityId": self.activityId}, Authorization=self.auth, X_Experience_API_Version="1.0")
        self.assertEqual(f_r.status_code, 400)
    ...
    def test_delete_set(self):
        ...
        put1 = self.client.put(path, teststatedelset1, content_type=self.content_type, Authorization=self.auth, X_Experience_API_Version="1.0")

        self.assertEqual(put1.status_code, 204)
        self.assertEqual(put1.content, '')
        
        r = self.client.get(self.url, testparamsdelset1, X_Experience_API_Version="1.0", Authorization=self.auth)
        self.assertEqual(r.status_code, 200)
        ...
        put1 = self.client.put(path, teststatedelset2, content_type=self.content_type, Authorization=self.auth, X_Experience_API_Version="1.0")

        self.assertEqual(put1.status_code, 204)
        self.assertEqual(put1.content, '')
        
        r = self.client.get(self.url, testparamsdelset2, X_Experience_API_Version="1.0", Authorization=self.auth)
        self.assertEqual(r.status_code, 200)
        state1_str = '%s' % teststatedelset2
        self.assertEqual(r.content, state1_str)
        self.assertEqual(r['etag'], '"%s"' % hashlib.sha1(state1_str).hexdigest())

        f_r = self.client.delete(self.url, {"registrationId": self.registrationId, "agent": self.testagent, "activityId": self.activityId}, Authorization=self.auth, X_Experience_API_Version="1.0")
    ...
    def test_ie_cors_put_delete(self):
        ...
        put1 = self.client.post(path, sparam1, content_type='application/x-www-form-urlencoded', X_Experience_API_Version="1.0")

        self.assertEqual(put1.status_code, 204)
        self.assertEqual(put1.content, '')
        
        r = self.client.get(self.url, {"stateId": sid, "activityId": self.activityId, "agent": testagent}, X_Experience_API_Version="1.0", Authorization=auth)
        self.assertEqual(r.status_code, 200)
        ...
        f_r = self.client.post(path, dparam, content_type='application/x-www-form-urlencoded', X_Experience_API_Version="1.0")
        self.assertEqual(f_r.status_code, 204)
    ...
    def test_agent_is_group(self):
        ...
        put1 = self.client.put(path, teststate1, content_type=self.content_type, Authorization=self.auth, X_Experience_API_Version="1.0")

        self.assertEqual(put1.status_code, 204)

        get1 = self.client.get(self.url, {"stateId":"group.state.id", "activityId": self.activityId, "agent":testagent}, X_Experience_API_Version="1.0", Authorization=auth)
        self.assertEqual(get1.status_code, 200)
        st = '%s' % teststate1
        self.assertEqual(get1.content, st)

        delr = self.client.delete(self.url, testparams1, Authorization=auth, X_Experience_API_Version="1.0")
    ...
    def test_post_new_state(self):
        ...
        r = self.client.post(path, json.dumps(state), content_type=self.content_type, Authorization=self.auth, X_Experience_API_Version="1.0")
        self.assertEqual(r.status_code, 204)
    ...
    def test_post_update_state(self):
        ...
        r = self.client.post(path, json.dumps(state2), content_type=self.content_type, Authorization=self.auth, X_Experience_API_Version="1.0")
        self.assertEqual(r.status_code, 204)
