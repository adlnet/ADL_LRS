...
class StatementsMoreTests(TestCase):
    ...
    def setUp(self):
        ...
        # Post statements
        post_statements = self.client.post(reverse(views.statements), json.dumps(stmt_list),content_type="application/json",HTTP_AUTHORIZATION=self.auth, X_Experience_API_Version="1.0")
    ...
    def test_unknown_more_id_url(self):
        moreURLGet = self.client.get(reverse(views.statements_more,kwargs={'more_id':'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa'}), X_Experience_API_Version="1.0",HTTP_AUTHORIZATION=self.auth )
        self.assertContains(moreURLGet, 'List does not exist - may have expired after 24 hours')

    def test_not_full_page_stmts(self):
        sincePostResponse = self.client.post(reverse(views.statements), {"until":self.secondTime},content_type="application/x-www-form-urlencoded", X_Experience_API_Version="1.0",HTTP_AUTHORIZATION=self.auth)
    ...
    def test_single_full_page_stmts(self):
        ...
        sincePostResponse = self.client.post(reverse(views.statements),
            ...
            content_type="application/x-www-form-urlencoded", X_Experience_API_Version="1.0",HTTP_AUTHORIZATION=self.auth)
        self.assertEqual(sincePostResponse.status_code, 200)
        rsp = sincePostResponse.content
    ...
    def test_single_full_second_not_full_more_stmts_url(self):
        sincePostResponse = self.client.post(reverse(views.statements), {"until":self.fourthTime},content_type="application/x-www-form-urlencoded", X_Experience_API_Version="1.0",HTTP_AUTHORIZATION=self.auth)
        self.assertEqual(sincePostResponse.status_code, 200)
        rsp = sincePostResponse.content
        ...
        moreURLGet = self.client.get(reverse(views.statements_more,kwargs={'more_id':resp_id}), X_Experience_API_Version="1.0",HTTP_AUTHORIZATION=self.auth)
    ...
    def test_two_pages_full_more_stmts_url(self):
        sincePostResponse = self.client.post(reverse(views.statements), {"until":self.fifthTime},content_type="application/x-www-form-urlencoded", X_Experience_API_Version="1.0",HTTP_AUTHORIZATION=self.auth)
        self.assertEqual(sincePostResponse.status_code, 200)
        rsp = sincePostResponse.content
        resp_json = json.loads(rsp)
        resp_url = resp_json['more']
    ...
    def test_two_pages_full_third_not_full_more_stmts_url(self):
        sinceGetResponse = self.client.get(reverse(views.statements), {"until":self.sixthTime, "limit":10}, X_Experience_API_Version="1.0",HTTP_AUTHORIZATION=self.auth)
        ...
        anotherURLGet = self.client.get(reverse(views.statements_more,kwargs={'more_id':more_resp_id}), X_Experience_API_Version="1.0",HTTP_AUTHORIZATION=self.auth)
    ...
    def test_limit_less_than_server_limit(self):
        sinceGetResponse = self.client.get(reverse(views.statements), {"until":self.sixthTime, "limit":8}, X_Experience_API_Version="1.0",HTTP_AUTHORIZATION=self.auth)
        self.assertEqual(sinceGetResponse.status_code, 200)
        rsp = sinceGetResponse.content
        resp_json = json.loads(rsp)
        resp_url = resp_json['more']
    ...
    def test_limit_more_than_server_limit(self):
        sinceGetResponse = self.client.get(reverse(views.statements), {"until":self.sixthTime, "limit":12}, X_Experience_API_Version="1.0",HTTP_AUTHORIZATION=self.auth)
        self.assertEqual(sinceGetResponse.status_code, 200)
        rsp = sinceGetResponse.content
        resp_json = json.loads(rsp)
        resp_url = resp_json['more']
    ...
    def test_two_pages_full_third_not_full_more_stmts_multiple_hits(self):
        ...
        sinceGetResponse = self.client.get(reverse(views.statements), {"until":self.sixthTime}, X_Experience_API_Version="1.0",HTTP_AUTHORIZATION=self.auth)
        self.assertEqual(sinceGetResponse.status_code, 200)
        rsp = sinceGetResponse.content        
        resp_json = json.loads(rsp)
        resp_url = resp_json['more']
        ...
        more2URLGet = self.client.get(reverse(views.statements_more, kwargs={'more_id':more_resp_id}), X_Experience_API_Version="1.0",HTTP_AUTHORIZATION=self.auth)
        ...
        # Simulate user clicking returned 'more' URL
        anotherMoreURLGet = self.client.get(reverse(views.statements_more,kwargs={'more_id':resp_id}), X_Experience_API_Version="1.0",HTTP_AUTHORIZATION=self.auth)
        ...
        # Simulate user clicking returned 'more' URL
        anotherMore2URLGet = self.client.get(reverse(views.statements_more, kwargs={'more_id':another_more_resp_id}), X_Experience_API_Version="1.0",HTTP_AUTHORIZATION=self.auth)
