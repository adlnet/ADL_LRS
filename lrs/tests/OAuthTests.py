...
class OAuthTests(TestCase):
	...
	def simple_test(self):
		...
		# Test request_token
		oauth_header_request_params = {'oauth_consumer_key': self.consumer.key,'oauth_signature_method': 'PLAINTEXT',
		...
		data = {'scope':'photos'}				
		request_resp = self.client.get("/TCAPI/OAuth/initiate/", Authorization=oauth_header_request_params, data=data)
		...
		# Test wrong scope
		data['scope'] = 'videos'
		scope_resp = self.client.get("/TCAPI/OAuth/initiate/", Authorization=oauth_header_request_params, data=data)
		...
		call_resp = self.client.get("/TCAPI/OAuth/initiate/", Authorization=oauth_header_request_params, data=data)
		...
		oauth_request = OAuthRequest.from_token_and_callback(access_token,
			http_url='http://testserver/TCAPI/OAuth/photo/', parameters=oauth_header_resource_params)
		...
		resp = self.client.get("/TCAPI/OAuth/photo/", Authorization=oauth_header_resource_params)
