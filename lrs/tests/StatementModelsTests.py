...
class StatementModelsTests(TestCase):
    ...
    def test_existing_stmtID_stmt(self):
        ...
        self.assertRaises(Exception, Statement.Statement, json.dumps({"object": {'id':'activity2'},
            "verb":"created", "statement_id":"blahID"}))
    ...
    def test_actor_stmt(self):
        ...
        stmt = Statement.Statement(json.dumps({"actor":{'objectType':'Person','name':['bob'],
            'mbox':['bob@example.com']}, "verb":"created","object": {"id":"activity5",
            "objectType": "Activity"}}))
    ...
    def test_authority_stmt(self):
        ...
        stmt = Statement.Statement(json.dumps({"authority":{'objectType':'Person','name':['bill'],
            'mbox':['bill@example.com']}, "verb":"created","object": {"id":"activity21",
            "objectType": "Activity"}}))
        activity = models.activity.objects.get(id=stmt.statement.stmt_object.id)
    ...
    def test_result_score_stmt(self):
        ...
        time = str(datetime.now())
        stmt = Statement.Statement(json.dumps({"actor":{'objectType':'Person','name':['jon'],
            'mbox':['jon@example.com']},"verb":"passed","object": {'id':'activity14'}, 
            "result": {'score':{'scaled':.95}, 'completion': True, 'success': True, 'response': 'yes',
            'duration': time,'extensions':{'key1': 'value1', 'key2':'value2'}}}))
        activity = models.activity.objects.get(id=stmt.statement.stmt_object.id)
        result = models.result.objects.get(id=stmt.statement.result.id)
    ...
    def test_no_registration_context_stmt(self):
        ...
        stmt = Statement.Statement(json.dumps({"verb":"failed","object": {'id':'activity14'},
                         'context': {'contextActivities': {'foo':'bar'}}})).statement
    ...
    def test_stmt_in_context_stmt(self):
        ...
        stmt = Statement.Statement(json.dumps({"verb":"kicked","object": {'id':'activity16'},
                'context':{'registration': guid, 'contextActivities': {'other': {'id': 'NewActivityID'}},
                'revision': 'foo', 'platform':'bar','language': 'en-US', 'statement': {'verb': 'graded',
                'object':{'id':'NestContextAct'}}}}))
    ...
    def test_instructor_in_context_stmt(self):
        ...
        stmt = Statement.Statement(json.dumps({"verb":"kicked","object": {'id':'activity17'},
                'context':{'registration': guid, 'instructor': {'objectType':'Person','name':['jon'],
                'mbox':['jon@example.com']},'contextActivities': {'other': {'id': 'NewActivityID'}},
                'revision': 'foo', 'platform':'bar','language': 'en-US', 'statement': {'verb': 'graded',
                'object':{'id':'NestContextAct1'}}}}))
    ...
    def test_actor_with_context_stmt(self):
        ...
        guid = str(uuid.uuid4())
        stmt = Statement.Statement(json.dumps({'actor':{'objectType':'Person', 'name': ['steve'],
            'mbox':['s@s.com']}, "verb":"kicked","object": {'id':'activity18'},'context':{'registration': guid,
            'instructor': {'objectType':'Person','name':['jon'],'mbox':['jon@example.com']},
    ...
    def test_agent_as_object(self):
        ...
        guid = str(uuid.uuid4())
        stmt = Statement.Statement(json.dumps({'object':{'objectType':'Agent', 'name': ['lulu'],
            'mbox':['lu@lu.com'], 'openid':['luluid'], 
            'account':[{'accountServiceHomePage':'http://lulu.com','accountName':'luluacct'}]}, "verb":"kicked"}))
    ...
    def test_stmt_as_object(self):
        ...
        guid = str(uuid.uuid4())
        stmt = Statement.Statement(json.dumps({"verb":"kicked", 'object':{'objectType':'Statement',
            'verb': 'punched', 'object': {'objectType':'activity', 'id':'testex.com'} }}))
