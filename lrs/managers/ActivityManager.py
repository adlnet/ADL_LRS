    ...
    def update_language_maps(self, incoming_act_def):
        ...
        # Else there was existing canonical data, and there in an incoming one, only update lang maps (name, desc, interaction activities)
            ...
            if 'scale' in incoming_act_def and 'scale' in self.activity.canonical_data['definition']:
                trans = {x['id']: x['description'] for x in incoming_act_def['scale']}
                for s in self.activity.canonical_data['definition']['scale']:
                    if s['id'] in trans:
                        s['description'] = dict(s['description'].items() + trans[s['id']].items())
            ...
            if 'source' in incoming_act_def and 'source' in self.activity.canonical_data['definition']:
                trans = {x['id']: x['description'] for x in incoming_act_def['source']}
            ...
            if 'target' in incoming_act_def and 'target' in self.activity.canonical_data['definition']:
                trans = {x['id']: x['description'] for x in incoming_act_def['target']}
