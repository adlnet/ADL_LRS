...
class StatementManager():
    ...
    def __init__(self, data, auth=None, define=True):
        ...
        if not isinstance(data, dict):
            ...
            self.data = data
        self.validate_statement_fields()
        self.populate()

    def validate_statement_fields(self):
        ...
        failed_list = [x for x in self.data.keys() if not x in allowed_fields]
        if failed_list:
    ...
    def check_statement_id(self):
        if 'id' in self.data:
            ...
            if not models.Statement.objects.filter(statement_id=stmt_id).exists():
                self.validate_incoming_uuid(stmt_id)
